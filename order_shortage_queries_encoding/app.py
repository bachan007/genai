...
I have an sql database named orders_db where i have 3 tables. 
...
Example2. If i want to select average ordered_qty of items from the orders table, i will write the query like "SELECT items,avg(ordered_qty) FROM ORDERS group by items"
...
when you join using two tables, always do the string formatting like making every record in same case like upper or lower,
...
def encode(text, db_map,table_map, column_map):
    encoded_text = text
    for db, code in db_map.items():
        encoded_text = encoded_text.replace(db, code)
    for table, code in table_map.items():
        encoded_text = encoded_text.replace(table, code)
    for column, code in column_map.items():
        encoded_text = encoded_text.replace(column, code)
    return encoded_text

def decode(encoded_text, db_map,table_map, column_map):
    decoded_text = encoded_text
    for db, code in db_map.items():
        decoded_text = decoded_text.replace(code,db)
    for table, code in table_map.items():
        decoded_text = decoded_text.replace(code,table)
    for column, code in column_map.items():
        decoded_text = decoded_text.replace(code,column)
    return decoded_text

db_map = {'orders_db':'db1'}
table_map = {"orders":"t1", "correlation_master":"t2", "rca":"t3"}
column_map = {"order_numbers":"C1", "items":"C2", "ordered_qty":"C3","delivered_qty":"C4",
              "rca_id":"C5",
              "rca":"C6"}

encoded_prompt = encode(prompt, db_map,table_map, column_map)
...
if submit:
    response = get_gemini_response(question,encoded_prompt)
    print(response)
    decoded_response = decode(response,db_map,table_map, column_map)
    data = read_sql_query(decoded_response,'orders.db')
